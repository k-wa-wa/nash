...
export interface TerminalOutputHandle {
    ...
    scrollToBottom: () => void;
    dispose: () => void;
    getBufferText: (lines?: number) => string;
        ...
        useImperativeHandle(
            ...
            () => ({
                ...
                getBufferText: (lines = 100) => {
                    ...
                    console.log(`[TerminalOutput] getBufferText: startLine=${startLine}, endLine=${endLine}, length=${trimmedResult.length}`);
                    if (trimmedResult.length > 0) {
                        console.log(`[TerminalOutput] Buffer preview (first 50): "${trimmedResult.substring(0, 50).replace(/\n/g, '\\n')}"`);
                        console.log(`[TerminalOutput] Buffer preview (last 50): "${trimmedResult.substring(Math.max(0, trimmedResult.length - 50)).replace(/\n/g, '\\n')}"`);
                    }
                ...
                registerCommandMarker: () => {
                    ...
                    console.log(`[TerminalOutput] registerCommandMarker at line: ${lastMarkerRef.current?.line}`);
                },
            }),
            [],
        );
