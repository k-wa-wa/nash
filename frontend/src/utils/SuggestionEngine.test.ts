...
describe("SuggestionEngine", () => {
	const mockHistory = ["git status", "ls -la", "docker ps", "cd /tmp"];
	...
	it("should return history only when input is empty", () => {
		...
		expect(result.some((s) => s.type === "static" && s.text === "git")).toBe(
			true,
		);
	});
	...
	it("should suggest top-level static commands and match prefix", () => {
		...
		expect(result.some((s) => s.type === "static" && s.text === "go")).toBe(
			true,
		);
	});
	...
	it("should return static suggestions for known command when there is a space", () => {
		...
		expect(result.some((s) => s.type === "static" && s.text === "status")).toBe(
			true,
		);
		expect(result.some((s) => s.type === "static" && s.text === "commit")).toBe(
			true,
		);
	});
	...
	it("should filter static subcommands based on partial argument", () => {
		...
		expect(result.some((s) => s.type === "static" && s.text === "push")).toBe(
			true,
		);
		expect(result.some((s) => s.type === "static" && s.text === "pull")).toBe(
			true,
		);
	...
	it("should mix history and static", () => {
		...
		const hasHistory = result.some(
			(s) => s.type === "history" && s.text === "git status",
		);
		const hasStatic = result.some(
			(s) => s.type === "static" && s.text === "status",
		);
